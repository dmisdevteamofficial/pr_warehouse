...
function explainSupabasePolicyHint(err) {
  ...
  if (
    ...
    return (
      'ตาราง pre_job_status เปิด RLS อยู่ แต่ยังไม่มี Policy อนุญาตให้ role: anon บันทึก/แก้ไข/ลบได้\n' +
      'ให้ไปที่ Supabase > Table Editor > pre_job_status > RLS แล้วสร้าง Policy สำหรับ SELECT/INSERT/UPDATE/DELETE (หรือปิด RLS ถ้าต้องการให้บันทึกได้เลย)'
    )
  }
...
async function fetchJobStatus() {
  ...
  try {
    const { data, error } = await supabase
      .from('pre_job_status')
      .select('id, job_name, created_by, created_at')
...
const filteredRows = computed(() => {
  ...
  return list.filter((r) => {
    const haystack = [r.job_name, creatorText(r)].filter(Boolean).join(' ').toLowerCase()
...
async function removeRow(row) {
  ...
  if (!ok) return
  try {
    const { error } = await supabase.from('pre_job_status').delete().eq('id', row.id)
    if (error) throw error
    await fetchJobStatus()
  } catch (err) {
...
<template>
  <div>
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      <div>
        <h1 class="text-[20px] font-semibold" style="color: var(--color-text-primary)">สถานะรับงาน</h1>
        <p class="text-[13px] mt-0.5" style="color: var(--color-text-muted)">รายงานข้อมูล </p>
      </div>
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-6 p-4 rounded-xl border" style="background: var(--color-bg-card); border-color: var(--color-border)">
      <div class="flex-1 relative">
        ...
        <input
          ...
          type="text"
          placeholder="ค้นหาสถานะ, ผู้บันทึก..."
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-[13px] min-w-[920px]">
          ...
          <tbody>
            ...
            >
              <td class="px-4 py-3 font-medium" style="color: var(--color-text-primary)">{{ row.job_name }}</td>
        ...
        <div class="relative w-full max-w-md h-full shadow-2xl flex flex-col" style="background: var(--color-bg-card)">
          <div class="px-6 py-4 border-b flex items-center justify-between" style="border-color: var(--color-border)">
            <h2 class="text-[16px] font-semibold" style="color: var(--color-text-primary)">
              {{ modalMode === 'edit' ? 'แก้ไขสถานะรับงาน' : 'เพิ่มสถานะรับงาน' }}
            </h2>
          ...
          <form class="flex-1 overflow-y-auto p-6 space-y-4" @submit.prevent="submit">
            <div class="space-y-1">
              <label class="text-[13px] font-medium" style="color: var(--color-text-primary)">สถานะรับงาน</label>
              <input
                ...
                placeholder="เช่น รอรับงาน / รับงานแล้ว ..."
              />
            </div>
          </form>
