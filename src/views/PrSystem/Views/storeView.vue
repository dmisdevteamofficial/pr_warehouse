...
function explainSupabasePolicyHint(err) {
  ...
  if (
    ...
    return (
      'ตาราง store เปิด RLS อยู่ แต่ยังไม่มี Policy อนุญาตให้ role: anon บันทึก/แก้ไข/ลบได้\n' +
      'ให้ไปที่ Supabase > Table Editor > store > RLS แล้วสร้าง Policy สำหรับ SELECT/INSERT/UPDATE/DELETE (หรือปิด RLS ถ้าต้องการให้บันทึกได้เลย)'
    )
  }
...
async function fetchStores() {
  ...
  try {
    const { data, error } = await supabase
      .from('store')
      .select('id, store_name, created_by, created_at')
...
const filteredRows = computed(() => {
  ...
  return list.filter((r) => {
    const haystack = [r.store_name, creatorText(r)].filter(Boolean).join(' ').toLowerCase()
...
<template>
  <div>
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      <div>
        <h1 class="text-[20px] font-semibold" style="color: var(--color-text-primary)">ร้านค้า</h1>
        <p class="text-[13px] mt-0.5" style="color: var(--color-text-muted)">บันทึกร้านค้าจากตาราง store</p>
      </div>
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-6 p-4 rounded-xl border" style="background: var(--color-bg-card); border-color: var(--color-border)">
      <div class="flex-1 relative">
        ...
        <input
          ...
          type="text"
          placeholder="ค้นหาร้านค้า, ผู้บันทึก..."
        ...
        <table class="w-full text-[13px] min-w-[920px]">
          <thead>
            <tr style="border-bottom: 1px solid var(--color-border)">
              <th class="text-left px-4 py-3 font-medium" style="color: var(--color-text-muted)">ชื่อร้านค้า</th>
          ...
          <tbody>
            ...
            >
              <td class="px-4 py-3 font-medium" style="color: var(--color-text-primary)">{{ row.store_name }}</td>
        ...
        <div class="relative w-full max-w-md h-full shadow-2xl flex flex-col" style="background: var(--color-bg-card)">
          <div class="px-6 py-4 border-b flex items-center justify-between" style="border-color: var(--color-border)">
            <h2 class="text-[16px] font-semibold" style="color: var(--color-text-primary)">
              {{ modalMode === 'edit' ? 'แก้ไขร้านค้า' : 'เพิ่มร้านค้า' }}
            </h2>
          ...
          <form class="flex-1 overflow-y-auto p-6 space-y-4" @submit.prevent="submit">
            <div class="space-y-1">
              <label class="text-[13px] font-medium" style="color: var(--color-text-primary)">ชื่อร้านค้า</label>
              <input
                ...
                placeholder="เช่น ร้าน A / ร้าน B ..."
              />
            </div>
          </form>
