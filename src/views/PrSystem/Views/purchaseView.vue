...
function explainSupabasePolicyHint(err) {
  ...
  if (
    ...
    return (
      'ตาราง purchase_status เปิด RLS อยู่ แต่ยังไม่มี Policy อนุญาตให้ role: anon บันทึก/แก้ไข/ลบได้\n' +
      'ให้ไปที่ Supabase > Table Editor > purchase_status > RLS แล้วสร้าง Policy สำหรับ SELECT/INSERT/UPDATE/DELETE (หรือปิด RLS ถ้าต้องการให้บันทึกได้เลย)'
    )
  }
...
async function fetchPurchaseStatuses() {
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('id, status_name, created_by, created_at')
    ...
    ui.showToast('โหลดข้อมูลสถานะการซื้อไม่สำเร็จ: ' + getErrorText(err), 'error')
    rows.value = []
  } finally {
    loading.value = false
  }
...
onMounted(fetchPurchaseStatuses)
...
const filteredRows = computed(() => {
  ...
  return list.filter((r) => {
    const haystack = [r.status_name, creatorText(r)].filter(Boolean).join(' ').toLowerCase()
...
function openEdit(row) {
  ...
  form.value = { status_name: row?.status_name ?? '' }
  isModalOpen.value = true
}
...
async function submit() {
  ...
  const name = (form.value.status_name || '').trim()
  if (!name) {
    ui.showToast('กรุณากรอกสถานะการซื้อ', 'warning')
    return
  }
  ...
  try {
    if (modalMode.value === 'edit') {
      ...
      const { error } = await supabase.from('purchase_status').insert(payload)
      if (error) throw error
      ui.showToast('บันทึกข้อมูลสำเร็จ', 'success')
    }

    closeModal()
    await fetchPurchaseStatuses()
  } catch (err) {
...
async function removeRow(row) {
  ...
  if (!ok) return
  try {
    const { error } = await supabase.from('purchase_status').delete().eq('id', row.id)
    if (error) throw error
...
<template>
  <div>
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      <div>
        <h1 class="text-[20px] font-semibold" style="color: var(--color-text-primary)">ตรวจสอบสินค้า</h1>
        <p class="text-[13px] mt-0.5" style="color: var(--color-text-muted)">บันทึกสถานะการซื้อจากตาราง purchase_status</p>
      </div>
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-6 p-4 rounded-xl border" style="background: var(--color-bg-card); border-color: var(--color-border)">
      <div class="flex-1 relative">
        ...
        <input
          ...
          type="text"
          placeholder="ค้นหาสถานะการซื้อ, ผู้บันทึก..."
        ...
        <table class="w-full text-[13px] min-w-[920px]">
          <thead>
            <tr style="border-bottom: 1px solid var(--color-border)">
              <th class="text-left px-4 py-3 font-medium" style="color: var(--color-text-muted)">สถานะการซื้อ</th>
          ...
          <tbody>
            ...
            >
              <td class="px-4 py-3 font-medium" style="color: var(--color-text-primary)">{{ row.status_name }}</td>
        ...
        <div class="relative w-full max-w-md h-full shadow-2xl flex flex-col" style="background: var(--color-bg-card)">
          <div class="px-6 py-4 border-b flex items-center justify-between" style="border-color: var(--color-border)">
            <h2 class="text-[16px] font-semibold" style="color: var(--color-text-primary)">
              {{ modalMode === 'edit' ? 'แก้ไขสถานะการซื้อ' : 'เพิ่มสถานะการซื้อ' }}
            </h2>
          ...
          <form class="flex-1 overflow-y-auto p-6 space-y-4" @submit.prevent="submit">
            <div class="space-y-1">
              <label class="text-[13px] font-medium" style="color: var(--color-text-primary)">สถานะการซื้อ</label>
              <input
                ...
                placeholder="เช่น รอตรวจสอบ / ผ่าน / ไม่ผ่าน ..."
              />
            </div>
          </form>
