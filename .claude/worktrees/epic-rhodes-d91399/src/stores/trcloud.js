...
export const useTrcloudStore = defineStore('trcloud', () => {
  ...
  const getRowsByType = (type) => {
    ...
    if (type === 'poItem') return poItemRows.value
  ...
  async function fetchTrcloudData(type = 'pr', options = {}) {
    ...
    const requestPromise = (async () => {
      ...
      if (type === 'pv') {
        ...
        endpoint = '/application/expense_report/api/engine-po/po_list.php'
        docType = ''
        useJson = true
      } else if (type === 'ap') {
      ...
      while (true) {
        ...
        for (const it of items) {
          ...
          const pid = it.po_number || it.po_id || it.pr_id || it.expense_id || it.payment_id || it.id || it.invoice_number || it.document_number
          if (pid && !seen.has(pid)) {
            seen.add(pid)
            newItems.push(it)
          }
        ...
        if (newItems.length > 0) {
          ...
          if (type === 'pr') prRows.value = [...results]
            ...
            const normalizedItems = results.map(x => {
              const rawStatus = (x.status || x.po_status || '').toString().toLowerCase()
              let status = x.status || 'ใหม่'
              if (rawStatus.includes('success') || rawStatus.includes('เสร็จสิ้น') || rawStatus.includes('สำเร็จ')) {
                status = 'เสร็จสิ้น'
              } else if (rawStatus.includes('partial') || rawStatus.includes('บางส่วน')) {
                status = 'รับบางส่วน'
              } else if (rawStatus.includes('confirm') || rawStatus.includes('ยืนยัน')) {
                status = 'ยืนยัน'
              } else if (rawStatus.includes('reject') || rawStatus.includes('ปฏิเสธ')) {
                status = 'ปฏิเสธ'
              } else if (rawStatus.includes('force') || rawStatus.includes('บังคับ')) {
                status = 'บังคับปิด'
              } else if (rawStatus.includes('new') || rawStatus.includes('ใหม่')) {
                status = 'ใหม่'
              }
              return { ...x, status }
            })
            poItemRows.value = normalizedItems
          }
  ...
  async function fetchAll(options = {}) {
    ...
    try {
      ...
      await Promise.all([
        ...
        fetchTrcloudData('poItem', { force, skipApStatusSync }),
  ...
  return {
    prRows, poRows, poItemRows, apRows, pvRows, expenseRows,
